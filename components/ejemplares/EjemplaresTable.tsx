  ...
  return (
    <div className="bg-neutral-900 rounded-lg overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.length === 0 && (
            <tr><td colSpan={7} className="p-4 text-center text-gray-400">Sin ejemplares</td></tr>
          )}
          ...
          {rows.map(r => (
            <tr key={r.id} className="border-t border-neutral-800">
              ...
              <td className="p-2">{r.estado_fisico}</td>
              <td className="p-2">{r.estatus}</td>
              <td className="p-2">{r.aula?.nombre ?? "—"}</td>
              <td className="p-2">
                <div className="flex gap-2">
                  <Guard perm="ejemplares:update" mode="disable">
                    <button className="px-3 py-1 rounded bg-neutral-700"
                      onClick={() => onEdit?.(r)}>Editar</button>
                  </Guard>
                  <Guard perm="ejemplares:delete" mode="disable">
                    <button className="px-3 py-1 rounded bg-red-600"
                      onClick={async () => {
                        ...
                        if (!onDelete) {
                          const res = await fetch("/api/ejemplares/delete", {
                            method: "DELETE",
