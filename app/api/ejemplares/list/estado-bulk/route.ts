...
import { splitAndNormalize, normalizeInv } from "@/utils/normalize";
...
export async function PATCH(req: Request) {
  try {
    ...
    // 1) Parseo original (para mostrar notFound con lo que pegó el usuario)
    ...
    const foundNormSet = new Set((existentes ?? []).map((r) => r.num_inventario_norm as string));
    const notFound = rawList.filter((raw, i) => !foundNormSet.has(normList[i]));
    ...
    // 5) Update por IDs (más seguro que por string)
    ...
    // 6) (Opcional) Historizar en 'movimientos' cuando enciendas el módulo
