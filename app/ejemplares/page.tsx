...
/** Mover ejemplares (aula/empleado) — sidebar */
  ...
  return (
    ...
    >
      <label className="text-sm text-neutral-300">
        Pega # de inventario a mover (acepta comas y saltos de línea).
      </label>
      <textarea
      ...
      <button
        type="submit"
        className="self-start px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60"
        disabled={loading}
      >
...
export default function EjemplaresPage() {
  ...
  const [open, setOpen] = useState<string | null>(null); // grupo abierto en "Ver más"
  ...
  const [editKey, setEditKey] = useState<string | null>(null); // sidebar
  ...
  const loadCombos = useCallback(async () => {
    const [aR, pR, eR] = await Promise.all([
      fetch("/api/aulas/list-lite", {
        cache: "no-store",
        credentials: "include",
      }),
      fetch("/api/items/list-lite", {
        cache: "no-store",
        credentials: "include",
      }),
      fetch("/api/employees/list-lite", {
        cache: "no-store",
        credentials: "include",
      }),
    ]);
  ...
  const loadDetails = useCallback(
    async (producto_id: string, aula_id: number | null) => {
      const url = `/api/ejemplares/list/by-group?producto_id=${producto_id}&aula_id=${
        aula_id ?? "null"
      }`;
      ...
      if (rows.length > 0) {
        if (ids.size === 1) {
          ...
          if (only === "null") label = "sin empleado";
            ...
            const emp = rows.find(
              (r: any) => (r.empleado?.id ?? r.empleado_id) === only
            )?.empleado;
            const name =
              emp?.nombre ??
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      {/* Tabla resumen: Empleado (grupo) + Total / Dañados / Disponibles */}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {groups.map((g) => {
              ...
              // disponibles = total - dañados (las bajas se deben excluir en SQL usando estatus <> 'retirado')
              const disponibles = Math.max(0, g.cantidad - g.danados);
              ...
              return (
                <tr key={key} className="border-t border-neutral-800">
                  ...
                  <td className="p-2">{disponibles}</td>
      ...
      {/* Detalle editable en MODAL centrado */}
      {open && (
        <div className="fixed inset-0 z-30 bg-black/60 flex items-center justify-center">
          <div className="bg-neutral-950 rounded-lg shadow-xl max-w-5xl w-full max-h-[70vh] flex flex-col border border-neutral-800">
            {/* Header modal */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800">
              <h2 className="font-semibold text-sm">
                Detalle de ejemplares ·{" "}
                {(() => {
                  const current = groups.find((g) => makeKey(g) === open);
                  if (!current) return "";
                  return `${current.producto} — ${current.aula}`;
                })()}
              </h2>
              <button
                type="button"
            ...
            {/* Cuerpo con tabla y scroll propio */}
      ...
      {/* Sidebar: Agregar ejemplares + mover ejemplares */}
      {editKey && (
        <div
                  ...
                  <section className="border border-neutral-800 rounded-lg">
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-xs text-neutral-400 mt-2">
                        Producto/aula/empleado precargados y bloqueados para
                        mantener coherencia del grupo.
                      </p>
                    </div>
                  </section>
...
function EditableRow({
  ...
  // Estado UI: bueno / regular / malo / inutilizable / baja (soft delete)
  async function handleEstadoChange(
    val: "bueno" | "regular" | "malo" | "inutilizable" | "baja"
  ) {
    const prevEstado = row.estado_fisico;
    const prevEstatus = row.estatus;

    let nuevoEstado = prevEstado;
    let nuevoEstatus: "activo" | "inactivo" | "retirado" = prevEstatus;

    if (val === "baja") {
      // Soft delete: marcamos como retirado
      nuevoEstatus = "retirado";
    } else {
      nuevoEstado = val;
      if (prevEstatus === "retirado") {
        // si sale de baja, vuelve a activo
        nuevoEstatus = "activo";
      }
    }
  ...
  // Valor que ve el usuario en el combo
  const uiEstado = row.estatus === "retirado" ? "baja" : row.estado_fisico;
  ...
  return (
    <tr className="border-t border-neutral-800 align-top">
      ...
      <td className="p-2">
        <select
          className="bg-neutral-800 rounded p-2 min-w-[160px]"
          value={uiEstado}
